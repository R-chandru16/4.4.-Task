...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val country = findViewById<TextInputLayout>(R.id.countryInputLayout)
        ...
        filterButton.setOnClickListener {
            ...
            CoroutineScope(Dispatchers.IO).launch{
                ...
                val selectedcountry = country.editText?.text?.toString()
                var count = 0;
                var booksList = mutableListOf<String>()
                decodedJsonResult.forEach {
                    if(it.author == author || it.language == selectedcountry) {
                        count++
                        if (count <= 3){
